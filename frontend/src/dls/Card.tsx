...
export function Card({ variant = 'default', interactive = false, padding = 'md', onClick, className = '', children }: CardProps) {
  ...
  if (onClick) {
    return (
      <button onClick={onClick} className={`w-full text-left active:brightness-95 ${base} ${className}`}>
        {children}
      </button>
    )
  }
