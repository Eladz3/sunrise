...
export function Drawer({ open, onClose, side = 'left', width = 'w-72', height = 'max-h-[85vh]', children, className = '' }: DrawerProps) {
  ...
  return createPortal(
    <>
      <div className={`fixed inset-0 z-40 bg-black/30 transition-opacity duration-300 ${visible ? 'opacity-100' : 'opacity-0'}`} onClick={onClose} />
      <div role="dialog" aria-modal="true" className={`fixed ${position} z-50 bg-white shadow-xl ${sizeClass} transition-transform duration-300 ease-out ${visible ? enter : exit} ${className}`}>
        {children}
      </div>
    </>,
    document.body
