...
export function Avatar({ displayName, profilePhoto, size = 'md', title, className = '' }: AvatarProps) {
  ...
  return (
    <div
      ...
      title={title}
      className={`${container} rounded-full overflow-hidden bg-gradient-to-br from-sunrise-400 to-dawn-500 flex items-center justify-center shrink-0 ${className}`}
    >
      {profilePhoto ? (
        <img src={profilePhoto} alt={displayName} className="w-full h-full object-cover" />
      ) : (
        <span className={`${text} text-white`}>{initial}</span>
      )}
    </div>
  );
}
