...
export const tests = {
  domainNotFound_updateGoal: () => runTest('domainNotFound_updateGoal', () => client.put('/api/goals/999999999', {}), { status: 404, code: 'DOMAIN_NOT_FOUND', messageContains: 'not found' }),

  domainNotFound_deleteGoal: () => runTest('domainNotFound_deleteGoal', () => client.delete('/api/goals/999999999'), { status: 404, code: 'DOMAIN_NOT_FOUND', messageContains: 'not found' }),

  domainNotFound_groupInvite: () => runTest('domainNotFound_groupInvite', () => client.post('/api/groups/999999999/invite?requestingUserId=1'), { status: 404, code: 'DOMAIN_NOT_FOUND', messageContains: 'not found' }),
  ...
  controllerCaught_invalidToken: () => runTest('controllerCaught_invalidToken', () => client.post('/api/groups/join/this-token-does-not-exist', { userId: 1 }), { status: 404, code: 'UNKNOWN' }),

  // GroupsController catches this itself and returns Forbid() — empty body,
  // bypasses the exception handler, so code lands as UNKNOWN.
  controllerCaught_forbid: (groupId = 1, nonOwnerUserId = 999999) => runTest('controllerCaught_forbid', () => client.delete(`/api/groups/${groupId}?userId=${nonOwnerUserId}`), { status: 403, code: 'UNKNOWN' }),

  runAll: async (): Promise<TestResult[]> => {
    const results = await Promise.all([tests.domainNotFound_updateGoal(), tests.domainNotFound_deleteGoal(), tests.domainNotFound_groupInvite(), tests.controllerCaught_invalidToken()])
